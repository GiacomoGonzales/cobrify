// Vercel Serverless Function para meta tags dinámicos de menús digitales (restaurante)
// Llamar con: /api/menu-meta?slug=mi-restaurante
...
export default async function handler(req, res) {
  ...
  console.log(`[MenuMeta] slug=${slug}, UA=${userAgent.substring(0, 50)}`)
  ...
  if (!isSocialBot(userAgent)) {
    return res.redirect(302, `/menu/${slug}`)
  }

  const business = await findBusinessByCatalogSlug(slug)

  if (!business) {
    const html = generateHTML({ name: 'Menú Digital' }, slug)
