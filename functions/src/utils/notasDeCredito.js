 ...
 *
 * Vive acá porque lo aplican los dos lados: la pantalla de la nota y el
 * "Reenviar" de Ventas (vía `src/utils/notasDeCredito.js`, que lo reexporta) y
 * el servidor justo antes de firmar, que es por donde pasan TODAS —la web, la
 * app con un bundle viejo y el reintento automático—. Las functions no pueden
 * importar de `src`; `src` sí puede importar de acá.
 */
...
/** Las notas vigentes de una factura, sin las anuladas y sin la que se está emitiendo. */
 ...
 * en pantalla se cuenta igual: se reenvía o se corrige con "Editar y
 * reemitir", y si se emite otra encima, al reenviarla quedan dos.
 */
...
/**
 * Por qué NO se puede emitir (o enviar) una nota de este monto para esta
 * factura, o null si se puede.
 *
 * `enCursoBloquea`: en PANTALLA (true) una nota en curso frena a la siguiente,
 * para que el usuario termine esa en vez de emitir otra. En el SERVIDOR, al
 * firmar (false), solo cuenta lo que SUNAT ya tiene: si hay dos en curso sale
 * la primera, y la segunda se frena sola cuando le toca, porque para entonces
 * la primera ya está aceptada. Así dos notas parciales legítimas no se
 * estorban entre sí.
 *
 ...
 */
export function motivoParaNoEmitirNota(factura, notas, totalNuevo, { enCursoBloquea = true } = {}) {
  ...
  if (enCursoBloquea && enCurso.length > 0) {
    const n = enCurso[0]
