...
export const generateCashReportExcel = async (sessionData, movements, invoices, businessData, deferredPayments = [], opts = {}) => {
  ...
  if (invoices.length > 0) {
    ...
    invoices.forEach(inv => {
      ...
      const statusText = isVoided ? 'Anulado' : (isNC ? 'Devolución' : (!cuentaEnCaja(inv) ? (inv.documentType === 'nota_venta' ? 'Convertida (no suma)' : 'Desde nota (no suma)') : (isPending ? 'Crédito' : (isPartial ? 'Parcial' : 'Pagado'))))
      const row = [
        inv.number || '-',
...
export const generateCashReportPDF = async (sessionData, movements, invoices, businessData, closedWithoutReceipt = [], orderModifications = [], deferredPayments = [], opts = {}) => {
  ...
  if (invoices.length > 0) {
    ...
    const getInvoiceStatus = (inv) => {
      ...
      if (inv.documentType === 'nota_venta' && inv.convertedTo && !cuentaEnCaja(inv)) return { label: 'Convertida', color: [37, 99, 235], excluded: true };
      // Convertido desde una nota que ya estaba cobrada, o parte de una: el dinero entró con la nota.
      if (!cuentaEnCaja(inv)) return { label: esParteDeNota(inv) ? 'Parte NV' : 'Desde NV', color: [100, 100, 100], excluded: true };
