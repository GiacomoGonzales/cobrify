/**
 * EMISIÓN MASIVA — motor común de emisión de lotes de GUÍAS.
 *
 * Salió del emisor de GRE Transportista cuando apareció el de GRE Remitente:
 * el circuito es idéntico y lo único que cambia son cuatro cosas (la colección
 * donde vive la guía, la función que la crea, la que la envía y el nombre de la
 * pantalla que se le nombra al usuario). Dos copias de este bucle se separan
 * solas con el primer arreglo que se haga en una sola.
 *
 ...
 *
 * IDEMPOTENTE por contenido: cada guía creada lleva un `bulkKey` (huella de la
 * operación). Si el usuario vuelve a subir el mismo archivo y emite de nuevo
 * —porque se cortó la luz a mitad del lote— las operaciones que ya tienen guía
 * se OMITEN en lugar de duplicarse.
 *
 * La creación usa la MISMA función con numeración atómica y el envío la MISMA
 * Cloud Function del botón individual: cero caminos nuevos hacia SUNAT.
 */
...
/**
 * Emite un lote de guías, una por una.
 *
 * @param {string} businessId
 * @param {Array}  operaciones - SOLO las operaciones sin errores del parser
 * @param {object} config
 * @param {string}   config.coleccion    - subcolección donde viven las guías
 * @param {string}   config.prefijoClave - prefijo del bulkKey ('gret_', 'grer_')
 * @param {Function} config.crearGuia    - (businessId, guia) => { success, id, number }
 * @param {Function} config.enviarGuia   - (businessId, id) => { success, accepted, description }
 * @param {string}   config.pantalla     - nombre de la pantalla para los mensajes
 * @param {object}   [config.extras]     - campos fijos que se suman a cada guía
 * @param {string}   [config.loteKey]
 * @param {string}   [config.branchId]
 * @param {Function} [config.onProgress] - ({ indice, total, nOperacion, etapa, numero })
 * @param {Function} [config.debeCancelar] - () => boolean; se consulta entre guías
 ...
 * reenvía desde su pantalla, nunca se duplica desde acá.
 */
export async function emitirLoteDeGuias(businessId, operaciones, {
  coleccion,
  prefijoClave,
  crearGuia,
  enviarGuia,
  ...
  const ref = collection(db, 'businesses', businessId, coleccion)
  ...
  for (let i = 0; i < total; i++) {
    ...
    const bulkKey = `${prefijoClave}${huellaDeOperacion(op)}`
    ...
    try {
      ...
      avisar('creando')
      const creacion = await crearGuia(businessId, {
        ...op.guia,
        ...extras,
        ...(branchId ? { branchId } : {}),
        bulkKey,
      ...
      const envio = await enviarGuia(businessId, creacion.id)
      ...
      if (envio.success && envio.accepted) {
        ...
        // La guía existe con estado pending: se reintenta desde su pantalla.
        // Reintentar acá arriesgaría el resto del lote.
        resultados.push({
          nOperacion: op.nOperacion,
          numero: creacion.number,
          estado: 'error_envio',
          mensaje: `${envio.error || 'No se pudo enviar.'} La guía ${creacion.number} quedó creada: reenvíala desde ${pantalla}.`,
        })
        avisar('error', { numero: creacion.number, mensaje: envio.error })
      }
    } catch (e) {
      console.error(`[LoteGuias/${coleccion}] Operación ${op.nOperacion}:`, e)
