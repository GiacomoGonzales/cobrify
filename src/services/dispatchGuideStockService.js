...
function computeBatchDeduction(productData, item, warehouseId, quantityToDeduct) {
  ...
  if (item.batchNumber) {
    ...
    if (idx !== -1) {
      ...
      console.warn(
        `[DispatchGuide] Lote "${item.batchNumber}" no encontrado para producto ${item.productId} ` +
